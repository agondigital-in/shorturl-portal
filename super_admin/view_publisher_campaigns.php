...
<head>
    ...
    <title>View Publisher Campaigns - Super Admin</title>
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<td>";
                                            echo "<a href='campaigns.php?toggle_status=".$row['id']."' class='btn btn-sm btn-".($row['status'] == 'active' ? 'warning' : 'success')."'>".($row['status'] == 'active' ? 'Deactivate' : 'Activate')."</a> ";
                                            echo "<a href='campaigns.php?delete=".$row['id']."' class='btn btn-sm btn-danger' onclick='return confirm(\"Are you sure you want to delete this campaign?\")'>Delete</a>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
                                    } else {
                                        echo "<tr><td colspan='10' class='text-center'>No campaigns found for this publisher</td></tr>";
                                    }
                                    $stmt->close();
                                    ?>
                                </tbody>
